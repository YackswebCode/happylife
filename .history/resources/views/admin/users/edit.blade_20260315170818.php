...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 text-dark-gray fw-bold">Edit User: {{ $user->name }}</h2>
        <div>
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST">
        ...
        <div class="row g-4">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="package_id" name="package_id">
                                ...
                                @foreach($packages as $pkg)
                                    <option value="{{ $pkg->id }}" {{ old('package_id', $user->package_id) == $pkg->id ? 'selected' : '' }}>
            ...
            <div class="col-md-12">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-0 pt-4">
                        <h5 class="fw-bold mb-0">Wallet Balances</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3">
                                <label for="commission_wallet_balance" class="form-label">Commission Wallet</label>
                                <input type="number" step="0.01" class="form-control" id="commission_wallet_balance" name="commission_wallet_balance" value="{{ old('commission_wallet_balance', $user->commission_wallet_balance) }}">
                            </div>
                            <div class="col-md-3">
                                <label for="registration_wallet_balance" class="form-label">Registration Wallet</label>
                                <input type="number" step="0.01" class="form-control" id="registration_wallet_balance" name="registration_wallet_balance" value="{{ old('registration_wallet_balance', $user->registration_wallet_balance) }}">
                            </div>
                            <div class="col-md-3">
                                <label for="shopping_wallet_balance" class="form-label">Shopping Wallet</label>
                                <input type="number" step="0.01" class="form-control" id="shopping_wallet_balance" name="shopping_wallet_balance" value="{{ old('shopping_wallet_balance', $user->shopping_wallet_balance) }}">
                            </div>
                            <div class="col-md-3">
                                <label for="rank_wallet_balance" class="form-label">Rank Wallet</label>
                                <input type="number" step="0.01" class="form-control" id="rank_wallet_balance" name="rank_wallet_balance" value="{{ old('rank_wallet_balance', $user->rank_wallet_balance) }}">
                            </div>
                        </div>
                    </div>
                </div>
