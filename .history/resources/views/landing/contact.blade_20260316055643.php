...
@section('title', 'Contact Us - ' . ($settings['site_name'] ?? 'Happylife Multipurpose Int\'l'))
        ...
        <div class="row g-5">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <div class="contact-card p-4 rounded-4 border-0 shadow-sm h-100">
                            ...
                            <a href="mailto:{{ $support->email ?? 'support@happylife.com' }}" class="text-danger fw-semibold text-decoration-none">
                                {{ $support->email ?? 'support@happylife.com' }}
                            </a>
                            <div class="mt-3">
                    ...
                    <div class="col-md-6">
                        <div class="contact-card p-4 rounded-4 border-0 shadow-sm h-100">
                            ...
                            <a href="tel:{{ $support->phone ?? '+2348000000000' }}" class="text-primary fw-semibold text-decoration-none">
                                {{ $support->phone ?? '+234 800 000 0000' }}
                            </a>
                            <div class="mt-3">
                    ...
                    <div class="col-12">
                        <div class="contact-card p-4 rounded-4 border-0 shadow-sm">
                            ...
                            <p class="text-muted mb-2">{{ $settings['contact_address'] ?? '123 Business Avenue, Victoria Island' }}</p>
                            <p class="text-muted mb-0">{{ $settings['contact_city'] ?? 'Lagos, Nigeria' }}</p>
                            <div class="mt-3">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-6">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span class="fw-semibold">{{ $hours['weekdays'] ?? '8:00 AM - 6:00 PM' }}</span>
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="d-flex justify-content-between">
                                                <span class="text-muted">Saturday:</span>
                                                <span class="fw-semibold">{{ $hours['saturday'] ?? '9:00 AM - 4:00 PM' }}</span>
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="d-flex justify-content-between">
                                                <span class="text-muted">Sunday:</span>
                                                <span class="fw-semibold">{{ $hours['sunday'] ?? '10:00 AM - 2:00 PM' }}</span>
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="d-flex justify-content-between">
                                                <span class="text-muted">Emergency:</span>
                                                <span class="fw-semibold text-danger">{{ $hours['emergency'] ?? '24/7' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <!-- Social Media (static, but you can make them dynamic via settings) -->
                    <div class="col-12">
            ...
            <div class="col-lg-7">
                <div class="contact-form-wrapper p-4 p-lg-5 rounded-4 shadow-lg">
                    ...
                    <!-- Contact Form SVG Illustration (unchanged) -->
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <select class="form-select @error('subject') is-invalid @enderror" id="subject" name="subject" required>
                                        ...
                                        <option value="registration" {{ old('subject') == 'registration' ? 'selected' : '' }}>Registration Issues</option>
                                        <option value="withdrawal" {{ old('subject') == 'withdrawal' ? 'selected' : '' }}>Withdrawal Issues</option>
                                        <option value="technical" {{ old('subject') == 'technical' ? 'selected' : '' }}>Technical Support</option>
                                        <option value="package" {{ old('subject') == 'package' ? 'selected' : '' }}>Package Inquiry</option>
                                        <option value="partnership" {{ old('subject') == 'partnership' ? 'selected' : '' }}>Business Partnership</option>
                                        <option value="other" {{ old('subject') == 'other' ? 'selected' : '' }}>Other Inquiry</option>
                                    </select>
                    ...
                    <div class="bg-white rounded-4 shadow-lg p-4 mx-auto" style="max-width: 400px;">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                <p class="text-muted small mb-0">{{ $settings['contact_address'] ?? '123 Business Avenue, Victoria Island' }}, {{ $settings['contact_city'] ?? 'Lagos' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
...
<style>
    ...
    .contact-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0,0,0,0.1) !important; }
    .contact-form-wrapper { background: linear-gradient(135deg, #ffffff 0%, #f8f9fa 100%); border: 1px solid #e9ecef; }
    .support-hours { background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); }
    .social-icon { transition: all 0.3s ease; border: 1px solid #dee2e6; }
    .social-icon:hover { background: linear-gradient(135deg, #E63323 0%, #d6281a 100%) !important; color: #fff !important; transform: translateY(-3px); border-color: #E63323; }
    .form-control, .form-select { border: 1px solid #dee2e6; transition: all 0.3s ease; }
    .form-control:focus, .form-select:focus { border-color: #E63323; box-shadow: 0 0 0 0.25rem rgba(230, 51, 35, 0.25); }
    .map-wrapper { border: 2px solid #e9ecef; }
    @media (max-width: 768px) {
        .display-4 { font-size: 2.5rem; }
        .display-5 { font-size: 2rem; }
        .py-6 { padding-top: 3rem !important; padding-bottom: 3rem !important; }
        .contact-form-wrapper { padding: 2rem !important; }
    }
