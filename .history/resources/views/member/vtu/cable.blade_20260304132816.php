   ...
   fetch(`{{ route('member.vtu.cable.plans') }}?provider_id=${providerId}`)
            ...
            .then(plans => {
                ...
                plans.forEach(plan => {
                    ...
                    option.value = plan.id || plan.plan_code; // fallback
    ...
    validateBtn.addEventListener('click', function(e) {
        ...
        if (!providerId) { alert('Please select a provider.'); return; }
        if (!smartCard) { alert('Please enter your smart card / IUC number.'); return; }
        if (!planId) { alert('Please select a bouquet.'); return; }
