...
<!-- Statistics Cards (updated to include rank wallet) -->
<div class="row mb-4 row-cols-1 row-cols-md-2 row-cols-xl-4 g-4">
    <!-- Total PV -->
    <div class="col">
    ...
    <!-- Shopping Wallet + Rank Wallet (combined card for space efficiency) -->
    <div class="col">
        <div class="card stat-card border-0 h-100">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h6 class="stat-label mb-2">SHOPPING WALLET</h6>
                        <h4 class="stat-value mb-2 text-success">₦{{ number_format($stats['shopping_balance'], 2) }}</h4>
                        <h6 class="stat-label mb-2">RANK WALLET</h6>
                        <h4 class="stat-value mb-0 text-purple">₦{{ number_format($stats['rank_balance'], 2) }}</h4>
                    </div>
...
<!-- Bonus Breakdown (including upgrade bonus) -->
<div class="row mb-4">
    <div class="col-12">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-0 py-3">
                    ...
                    <div class="col-md-6 col-lg-4 mb-3">
                        <div class="card border h-100">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <h6 class="mb-1">Upgrade Bonus</h6>
                                        <small class="text-muted">From upgrades</small>
                                    </div>
                                    <span class="badge bg-teal">₦{{ number_format($user->upgrade_bonus_total ?? 0, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="row">
    ...
    <!-- Rank Details (updated progress based on weaker side) -->
    <div class="col-lg-6 mb-4">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white border-0 py-3">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <!-- Next Rank Progress (based on weaker side PV) -->
                        @if($nextRank)
                        @php
                            $weakerSidePv = min($user->left_pv ?? 0, $user->right_pv ?? 0);
                            $progress = $nextRank->required_pv > 0 ? min(($weakerSidePv / $nextRank->required_pv) * 100, 100) : 0;
                        @endphp
                        <div class="mt-4">
                ...
                <div class="text-center py-5">
                    ...
                    <!-- Show first rank as target if no rank achieved (based on weaker side) -->
                    @if($nextRank)
                    @php
                        $weakerSidePv = min($user->left_pv ?? 0, $user->right_pv ?? 0);
                        $progress = $nextRank->required_pv > 0 ? min(($weakerSidePv / $nextRank->required_pv) * 100, 100) : 0;
                    ...
                    <div class="mt-4">
                        ...
                        <small class="text-muted mt-2 d-block">
                            Left: {{ number_format($user->left_pv ?? 0, 1) }} PV / {{ number_format($nextRank->required_pv, 1) }} PV needed<br>
                            Right: {{ number_format($user->right_pv ?? 0, 1) }} PV / {{ number_format($nextRank->required_pv, 1) }} PV needed
                        </small>
                    </div>
                    @endif
                </div>
