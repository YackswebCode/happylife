...
class VTUController extends Controller
{
    ...
    public function purchaseAirtime(Request $request)
    {
        ...
        try {
            ...
            // Record VTU transaction with API response (array will be cast to JSON)
            VtuTransaction::create([
                ...
                'api_response' => $apiData, // ✅ array – casted to JSON by model
            ]);
    ...
    // ------------------------------------------------------------------------
    // Data, Cable, Electricity, and AJAX methods remain the same as before
    // (unchanged, but included for completeness)
    // ------------------------------------------------------------------------

    public function data()
    {
        $networks = VtuProvider::where('category', 'data')->where('is_active', true)->get();
        $recentData = VtuTransaction::where('user_id', Auth::id())->where('service_type', 'data')->latest()->take(5)->get();
...
public function purchaseData(Request $request)
{
    ...
    $user = Auth::user();
    $phone = $request->phone;
    $plan = VtuPlan::with('provider')->findOrFail($request->plan_id);
    $amount = $plan->amount; // Use plan's amount, ignore submitted amount for security

    // Get the network provider
    $provider = $plan->provider;
    $networkCode = strtolower($provider->name); // e.g., "mtn"
    ...
    // Check user's commission wallet balance
    if ($user->commission_wallet_balance < $amount) {
        return back()->with('error', 'Insufficient commission wallet balance.');
    }

    // Get or create the commission wallet record
    $commissionWallet = Wallet::firstOrCreate(
        ['user_id' => $user->id, 'type' => 'commission'],
        ['balance' => $user->commission_wallet_balance, 'locked_balance' => 0]
    );
    ...
    $payload = [
        'plan'      => $plan->api_code,   // e.g., "PSPLAN_177"
        'recipient' => $phone,
        'network'   => $networkCode,
        'ref'       => $ref,
    ];
    ...
    try {
        ...
        // Deduct from wallets table record
        $commissionWallet->balance -= $amount;
        $commissionWallet->save();
        ...
        return redirect()->route('member.vtu.data')
            ->with('success', 'Data bundle purchased successfully! ' . $plan->name . ' sent to ' . $phone);
    ...
    {
        $cableProviders = VtuProvider::where('category', 'cable')->where('is_active', true)->get();
        $recentCable = VtuTransaction::where('user_id', Auth::id())->where('service_type', 'cable')->latest()->take(5)->get();
    ...
    public function purchaseCable(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('member.vtu.cable')->with('success', 'Cable TV subscription successful!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $discos = VtuProvider::where('category', 'electricity')->where('is_active', true)->get();
        $recentElectricity = VtuTransaction::where('user_id', Auth::id())->where('service_type', 'electricity')->latest()->take(5)->get();
    ...
    public function purchaseElectricity(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('member.vtu.electricity')->with('success', 'Electricity bill payment successful!');
        } catch (\Exception $e) {
            DB::rollBack();
            return back()->with('error', 'Transaction failed.');
        }
