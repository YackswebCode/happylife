...
class VTUController extends Controller
{
    ...
    public function purchaseAirtime(Request $request)
    {
        ...
        try {
            ...
            if ($response->failed()) throw new \Exception('API request failed with status ' . $response->status());
    ...
    public function validateSmartCard(Request $request)
    {
        ...
        $apiSettings = VtuApiSetting::where('is_active', true)->first();
        if (!$apiSettings) {
            return response()->json(['error' => 'VTU API not configured'], 500);
        }
        ...
        try {
            ...
            // Check if API returned invalid smart card
            if (!isset($data['status']) || $data['status'] !== true) {
                return response()->json(['error' => 'Smart card number is incorrect'], 422);
            }
    ...
    public function purchaseCable(Request $request)
    {
        ...
        try {
            ...
            if (!isset($apiData['status']) || $apiData['status'] !== true) {
                return back()->with('error', $apiData['description'] ?? 'Smart card transaction failed');
            }
