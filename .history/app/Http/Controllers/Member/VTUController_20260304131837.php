    ...
    try {
        ...
        if (isset($data['message']['details'])) {
            $plans = collect($data['message']['details'])->map(function ($plan) {
                return [
                    'id'     => $plan['id'],          // This is the plan_id to use in vend
                    'name'   => $plan['name'],
                    'amount' => $plan['amount'],
                ];
            })->values();
...
public function validateSmartCard(Request $request)
{
    $request->validate([
        ...
        'plan_id'     => 'required|string', // the API plan ID from bouquets
    ]);
    ...
    // The API expects month parameter (default 1)
    $payload = [
        'service'  => $serviceCode,
    ...
    try {
        ...
        if ($response->failed()) {
            return response()->json(['error' => 'Validation request failed'], 500);
        }
        ...
        // Return customer name and any other info
...
public function purchaseCable(Request $request)
{
    $request->validate([
        ...
        'smart_card'    => 'required|string',
        'plan_id'       => 'required|string', // the API plan ID (the encrypted ID from bouquets)
        'amount'        => 'required|numeric|min:1',
        'customer_name' => 'required|string', // from validation step
    ]);
    ...
    $payload = [
        ...
        'service'       => $serviceCode,
        'ref'           => $ref,
        'phone'         => $user->phone, // or optionally from request
        'email'         => $user->email,
        'month'         => 1, // default 1 month
    ];
