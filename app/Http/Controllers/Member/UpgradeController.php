...
class UpgradeController extends Controller
{
    ...
    public function process(Request $request)
    {
        ...
        try {
            ...
            // 2. Update cached shopping_wallet_balance on users table
            $user->shopping_wallet_balance = $shoppingWallet->balance;
            $user->save();

            // 3. Record wallet transaction for the payment
            ...
            // ========== UPGRADE BONUSES ==========
            $bonusAmount = $newPackage->upgrade_bonus_amount ?? 0;

            if ($bonusAmount > 0) {
                // Bonus to the upgrading user themselves
                $user->commission_wallet_balance += $bonusAmount;
                $user->upgrade_bonus_total += $bonusAmount;
                $user->save();
                ...
                WalletTransaction::create([
                    ...
                    'type'        => 'credit',
                    'amount'      => $bonusAmount,
                    'description' => 'Upgrade bonus for upgrading to ' . $newPackage->name,
                    'reference'   => $reference . '-SELF',
                    'status'      => 'completed',
                ]);

                // Bonus to the sponsor (if exists)
                if ($user->sponsor_id) {
                    $sponsor = User::find($user->sponsor_id);
                    if ($sponsor) {
                        $sponsor->commission_wallet_balance += $bonusAmount;
                        $sponsor->upgrade_bonus_total += $bonusAmount;
                        $sponsor->save();
                        ...
                        WalletTransaction::create([
                            ...
                            'type'        => 'credit',
                            'amount'      => $bonusAmount,
                            'description' => 'Upgrade bonus from downline ' . $user->username . ' upgrading to ' . $newPackage->name,
